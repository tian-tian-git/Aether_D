...
│   ├── spec/v0.1/             # 语言规范(语法/语义/类型/契约/诊断/标准库)
│   ├── guides/                # 人类指南 + AI 指南(使用与维护)
│   ├── bench/                 # M2-M6 全部基准与实验报告
│   └── adr/                   # 架构决策记录
├── crates/                    # Rust workspace(子项目)
│   ├── aether-ast/            # AST / NodeId / Span / 规范打印
│   ├── aether-diagnostic/     # 结构化诊断(JSON)
│   ├── aether-parser/         # 词法 + 递归下降解析
│   ├── aether-verify/         # 类型检查 + Z3 静态契约验证
│   ├── aether-interp/         # 树遍历解释器(值模型/内建)
│   ├── aether-vm/             # 字节码编译器 + VM(默认后端)
│   └── aether-cli/            # parse / check / run / repl
├── lib/aether-std.ae          # 用 Aether 自身实现的标准库(自举)
├── examples/                  # Aether 示例(含坦克大战、动态艺术字)
